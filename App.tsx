...
const TransactionPage: React.FC<TransactionPageProps> = ({ type, transactions, categories, accounts, onAdd, onUpdate, onDelete }) => {
  ...
  // If it's the INCOME page, we MUST exclude credit card repayments (Income on a Credit account)
    ...
    <div className="flex min-h-screen">
      ...
      <aside className={`fixed inset-y-0 left-0 bg-white/95 backdrop-blur-md border-r border-slate-200 p-6 flex flex-col z-50 transition-all duration-300 transform ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'} lg:translate-x-0 lg:static lg:h-screen print:hidden ${isCollapsed ? 'lg:w-24' : 'lg:w-64'}`}>
        <div className={`mb-10 flex items-center ${isCollapsed ? 'justify-center' : 'justify-between'}`}>
          <div className="flex items-center space-x-2 overflow-hidden">
            ...
            {!isCollapsed && <span className="text-lg font-black text-slate-800 tracking-tight truncate">{farmName}</span>}
          </div>
        ...
        <div className="mt-auto pt-6 border-t border-slate-100">
          <div className={`flex items-center space-x-3 mb-4 ${isCollapsed ? 'justify-center' : 'px-2'}`}>
            ...
            {/* Fix: changed handleLogout to onLogout which is the prop name passed to Layout */}
      ...
      <main className="flex-1 min-w-0">
        <header className="hidden lg:flex mb-8 justify-between items-center px-8 pt-8">
          <div><h1 className="text-2xl font-bold text-slate-900">{getTitle()}</h1><p className="text-slate-500 text-sm">Isolated Account: <span className="font-bold text-emerald-600">{user.email}</span></p></div>
        </header>
...
const App: React.FC = () => {
  ...
  useEffect(() => {
    if (user) {
      ...
      // RESET local data immediately when user changes to prevent bleeding
      // while we wait for the cloud fetch
      ...
      const loadFromCloud = async () => {
        ...
        if (cloudData) {
          ...
          // If new user, set an initial personalized name
  ...
  return (
    <Router>
      <Layout user={user} farmName={data.farmName} onLogout={handleLogout} sidebarConfig={data.sidebarConfig} isCollapsed={data.isSidebarCollapsed} onToggleCollapse={() => updateData({ isSidebarCollapsed: !data.isSidebarCollapsed })} isSyncing={isSyncing}>
        <Routes>
